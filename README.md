...
- **Systemd-first:** `/opt/daymind` hosts the repo plus `/opt/daymind/venv`; `/opt/daymind/runtime/` stores text artifacts like `ledger.beancount`. `/etc/default/daymind` ships with safe defaults (`APP_HOST=127.0.0.1`, `APP_PORT=8000`, `FAVA_PORT=8010`, `LEDGER_FILE=/opt/daymind/runtime/ledger.beancount`, `PYTHONPATH=/opt/daymind`) so both services stay environment-aware. `scripts/remote_deploy.sh` rebuilds the venv if missing, runs `pip install -r requirements.txt && pip install -e .`, sanity-checks `uvicorn`/`fava`, and seeds the ledger file before reloading units (see `DEPLOY.md` for the full runbook).
- **Services:** `daymind-api` runs `/opt/daymind/venv/bin/uvicorn src.api.main:app --host ${APP_HOST:-127.0.0.1} --port ${APP_PORT:-8000}` behind systemd, while `daymind-fava` launches `/opt/daymind/scripts/start_fava.sh`, which exports `PYTHONPATH`, ensures the ledger exists, and execs `/opt/daymind/venv/bin/fava` on `127.0.0.1:8010`. Both units specify `WorkingDirectory=/opt/daymind`, `EnvironmentFile=/etc/default/daymind`, and `Restart=on-failure`; `infra/systemd/checks/healthcheck.sh` prints the last 80 log lines if any unit drops to aid CI/ops triage (`journalctl -u daymind-api -f` / `daymind-fava` remains the go-to live tail).
- **CI/CD deploys:** `.github/workflows/ci_cd.yml` runs `terraform_apply → deploy_app → verify_services`. The deploy step calls `scripts/remote_deploy.sh`, enforces `systemctl enable --now daymind-api daymind-fava`, and curls `http://127.0.0.1:8000/healthz` + `/metrics`. The `verify_services` job then executes `infra/systemd/checks/healthcheck.sh` over SSH so CI logs include Redis/API/Fava status plus any recent journal output before declaring the pipeline green.
