...
- **Systemd-first:** Copy the repo to `/opt/daymind`, install the units in `infra/systemd/`, and keep runtime env vars in `/etc/default/daymind`. Deploys rebuild `/opt/daymind/venv`, reload `daymind-api` / `daymind-fava`, and tailing `journalctl -u daymind-api -f` (or `daymind-fava`) is the fastest way to diagnose issues. `DEPLOY.md` documents provisioning, smoke tests, firewall, rollback, and the optional Docker Compose path.
