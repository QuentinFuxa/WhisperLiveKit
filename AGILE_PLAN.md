...
- **US-13.1 – Recording + Chunk Uploader** — 🛠 In Progress (Jetpack Compose MVP screen with record toggle/pending queue counter, notification-backed ForegroundService writing mono 16 kHz ~6 s WAV chunks, Retrofit + WorkManager uploader w/ exponential backoff and `X-API-Key`, EncryptedSharedPreferences/`local.properties` config fallbacks).
