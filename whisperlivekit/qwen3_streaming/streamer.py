"""Cached full-hypothesis streamers for the Qwen3 streaming backend.

``CachedFullHypothesisStreamer`` appends finalized audio embeddings to a
per-session cache, regenerates a greedy full-text hypothesis per chunk, and
commits only stable word prefixes. ``SegmentedCachedFullHypothesisStreamer``
bounds decode cost for long sessions by periodically finalizing a segment and
