...
def visualize_tokens(tokens, speaker_names=None):
    ...
    for entry in conversation:
        speaker_display = (
            speaker_names.get_name(entry["speaker"])
            if speaker_names
            else entry["speaker"]
        )
        print(
            f"Speaker {speaker_display}: {entry['text']}"
        )  # Adds logic for speaker names in the conversation
