...
from whisperlivekit.whisper.audio import (
    N_FRAMES,
    N_SAMPLES,
    TOKENS_PER_SECOND,
    log_mel_spectrogram,
...
def load_coreml_encoder():
    ...
    COREML_ENCODER_PATH = os.environ.get(
        "MLCORE_ENCODER_PATH", "whisperlivekit/whisper/whisper_encoder.mlpackage"
    )
    _coreml_encoder = MLModel(COREML_ENCODER_PATH)
    spec = _coreml_encoder.get_spec()
    _coreml_input_name = (
        spec.description.input[0].name if spec.description.input else "mel"
    )
    _coreml_output_name = (
        spec.description.output[0].name if spec.description.output else None
    )
...
class AlignAtt:
    ...
    def init_context(self):
        ...
        if self.cfg.static_init_prompt is not None:
            self.state.context = TokenBuffer.from_text(
    ...
    def logits(
        ...
        if self.state.decoder_type == "greedy":
            ...
            return self.state.inference.logits(
                tokens, audio_features, return_cross_attn=return_cross_attn
            )
    ...
    def insert_audio(self, segment=None):
        ...
        while len(self.state.segments) > 1 and segments_len > self.cfg.audio_max_len:
            ...
            self.state.cumulative_time_offset += (
                removed_len  # Track cumulative time removed
            )
    ...
    def lang_id(self, encoder_features):
        ...
        x = torch.tensor([[self.tokenizer.sot]] * n_audio).to(
            self.model.device
        )  # [n_audio, 1]
    ...
    def infer(self, is_last=False):
        ...
        beg_encode = time()
        if self.use_mlcore:
            coreml_encoder, coreml_input_name, coreml_output_name = (
        ...
        if self.mlx_encoder:
            ...
            if (
                self.device == "cpu"
            ):  # it seems that on gpu, passing StorageView to torch.as_tensor fails and wrapping in the array works
            ...
            try:
                ...
                TypeError
            ):  # Normally the cpu condition should prevent having exceptions, but just in case:
                encoder_feature = torch.as_tensor(
        ...
        while (
            not completed and current_tokens.shape[1] < self.max_text_len
        ):  # bos is 3 tokens
            ...
            result = self.logits(
                tokens_for_logits, encoder_feature, return_cross_attn=True
            )
            ...
            if new_segment and self.tokenizer.no_speech is not None:
                probs_at_sot = (
                    logits[:, self.state.sot_index, :].float().softmax(dim=-1)
                )
            ...
            logger.debug(
                f"Decoding completed: {completed}, sum_logprobs: {sum_logprobs.tolist()}, tokens: "
            )
            self.debug_print_tokens(current_tokens)
            ...
            most_attended_frames = torch.argmax(
                attn_of_alignment_heads[:, -1, :], dim=-1
            )
            ...
            logger.debug(
                f"Absolute timestamps: {absolute_timestamps} (offset: {self.state.cumulative_time_offset:.2f}s)"
            )
            ...
            if (
                ...
                if current_tokens.shape[1] > 1 and current_tokens[0, -2] >= DEC_PAD:
                    ...
                    self.state.last_attend_frame = -self.cfg.rewind_threshold
                    current_tokens = (
                        torch.cat(self.state.tokens, dim=1)
        ...
        if self.state.pending_incomplete_tokens:
            logger.debug(
                f"[UTF-8 Fix] Prepending {len(self.state.pending_incomplete_tokens)} pending tokens: {self.state.pending_incomplete_tokens}"
            )
        ...
        for word, word_tokens in zip(split_words, split_tokens):
            ...
            if replacement_char in word:
                logger.warning(
                    f"[UTF-8 Filter] Skipping incomplete word from client output: {repr(word)}"
                )
        ...
        if split_words and replacement_char in split_words[-1]:
            ...
            logger.warning(
                f"[UTF-8 Fix] Holding {len(self.state.pending_incomplete_tokens)} incomplete tokens for next chunk: {self.state.pending_incomplete_tokens}"
            )

        return timestamped_words

    def _process_cross_attention(
        self, cross_attns: List[torch.Tensor], content_mel_len: int
    ) -> torch.Tensor:
        """
        ...
        if cross_attns and isinstance(cross_attns[0], list):
            flattened_attns: List[torch.Tensor] = [
        ...
        if not tmp:
            return torch.zeros(
                self.cfg.beam_size, 1, content_mel_len, device=self.device
            )
