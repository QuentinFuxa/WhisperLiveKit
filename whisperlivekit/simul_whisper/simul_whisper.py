...
class AlignAtt:
    ...
    def __init__(
        ...
        mlx_encoder=None,
        fw_encoder=None,
        encoder_batcher=None,
    ) -> None:
        # Shared model reference (can be shared across sessions)
        self.model = loaded_model
        self.mlx_encoder = mlx_encoder
        self.fw_encoder = fw_encoder
        self.encoder_batcher = encoder_batcher  # Optional EncoderBatcher for multi-session batching
        if fw_encoder:
    ...
    async def infer_async(self, is_last=False):
        """
        Async version of infer() that uses EncoderBatcher for batched encoding.

        This method should be used when encoder_batcher is set, allowing multiple
        sessions to batch their encoder passes together for GPU efficiency.

        Falls back to synchronous infer() if encoder_batcher is not set.
        """
        if self.encoder_batcher is None:
            # No batcher configured, use synchronous path
            return self.infer(is_last=is_last)
        ...
        # Use batched encoder - this is the key difference from sync infer()
        beg_encode = time()
        encoder_feature, content_mel_len = await self.encoder_batcher.encode(input_segments)
        end_encode = time()
        logger.debug(f"Batched encoder duration: {end_encode - beg_encode:.4f}s")

        # Rest of the method is identical to infer() - handle language detection and decoding
        if self.cfg.language == "auto" and self.state.detected_language is None and self.state.first_timestamp:
            seconds_since_start = self.segments_len() - self.state.first_timestamp
            if seconds_since_start >= 2.0:
                language_tokens, language_probs = self.lang_id(encoder_feature)
