...
class SimulStreamingOnlineProcessor:
    ...
    def insert_audio_chunk(self, audio: np.ndarray, audio_stream_end_time):
        ...
        self.end = (
            audio_stream_end_time  # Aligned with whisperstreaming backend behavior
        )
    ...
    def process_iter(self, is_last=False) -> Tuple[List[ASRToken], float]:
        ...
        try:
            ...
            if (
                self.model.cfg.language == "auto"
                and timestamped_words[0].detected_language is None
            ):
...
class SimulStreamingASR:
    ...
    def __init__(self, logfile=sys.stderr, **kwargs):
        ...
        self.pytorch_path, compatible_whisper_mlx, compatible_faster_whisper = (
            None,
            True,
            True,
        )
        if self.model_path:
            ...
            self.pytorch_path, compatible_whisper_mlx, compatible_faster_whisper = (
                model_path_and_type(resolved_model_path)
            )
            ...
            raise ValueError(
                "Either model_size or model_path must be specified for SimulStreaming."
            )
    ...
    def _resolve_encoder_backend(
        self, preferred_backend, compatible_whisper_mlx, compatible_faster_whisper
    ):
        ...
        if choice == "mlx-whisper":
            if not self._can_use_mlx(compatible_whisper_mlx):
                raise RuntimeError(
                    "mlx-whisper backend requested but MLX Whisper is unavailable or incompatible with the provided model."
                )
            return "mlx-whisper"
        if choice == "faster-whisper":
            if not self._can_use_faster(compatible_faster_whisper):
                raise RuntimeError(
                    "faster-whisper backend requested but Faster-Whisper is unavailable or incompatible with the provided model."
                )
            return "faster-whisper"
        if choice == "openai-api":
            raise ValueError(
                "openai-api backend is only supported with the LocalAgreement policy."
            )
        # auto mode
        ...
        if warmup_audio is not None:
            ...
            if self.fast_encoder:
                ...
                whisper_model.transcribe(
                    warmup_audio, language=self.lan if self.lan != "auto" else None
                )
