...
class Line(TimedText):
    ...
    def to_dict(
        self, speaker_names: Optional["SpeakerNameManager"] = None
    ) -> Dict[str, Any]:
        """Serialize the line for frontend consumption."""
        speaker_id = int(self.speaker) if self.speaker != -1 else 1
        speaker_display = (
            speaker_names.get_name(speaker_id) if speaker_names else str(speaker_id)
        )

        _dict: Dict[str, Any] = {
            "speaker": speaker_id,
            "speaker_name": speaker_display,  # Add custom name or keep default number
            "text": self.text,
